...
- Functions -  ex - `Function celsiusToFar : REAL
                     ...
                     END_VAR
                     celsiusToFar :=  celsius * 1.8 +32;  ` - is the return stmt
        - inside that we can also define the var for the function scoped just like the var i/p param. 
...
**TF6 Connectivity** - uses the json format for the Connectivity, used when we want to communicate b/w the tc and the external app
    - **opc ua** also used for the data exchange b/w s/ms
    - **modbus Tcp** - 
    - **TC3 Profinet RT device** - enable our plc to talk profinet(is a field bus developed by the siemens).
    - **TC3 FTP Client** enables our tc to talk to the ftp, to upload and download files.
...
- The better way to read the var is via Access Symbolic data by instance/symbol path (dotnet code refer the beckhoff documentation)
- 